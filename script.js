...
// --- Audio Mode Elements --- Removed

// --- Preload Audio Files ---
const numberAudio = {};
let audioPreloaded = false;

function preloadNumberAudio() {
    if (audioPreloaded) return;
    console.log("Preloading number audio...");
    try {
        for (let i = 1; i <= 20; i++) {
            // Assumes audio files are in 'audio/numbers/' folder
            numberAudio[i] = new Audio(`audio/numbers/${i}.mp3`);
            numberAudio[i].preload = 'auto'; // Hint to the browser to preload
             numberAudio[i].onerror = () => console.error(`Failed to load audio: audio/numbers/${i}.mp3`);
        }
        audioPreloaded = true;
        console.log("Number audio preloading initiated.");
    } catch (e) {
        console.error("Error creating number audio objects:", e);
    }
}
// --- End Preload Audio --- 
...
function onPointerDown(event) {
    ...
    for (const intersect of intersects) {
        ...
        if (clickedObject.userData.isAnimal && !alreadyClicked) {
            ...
            // --- Play Sound if Checkbox Checked --- Modified Block ---
            if (playSoundCheckbox.checked) {
                playNumberAudio(currentCount);
            }
            // --- End Play Sound --- 
...
function playNumberAudio(number) {
    const sound = numberAudio[number];
    if (sound) {
        sound.currentTime = 0; // Rewind
        sound.play().catch(e => console.error(`Error playing number audio ${number}:`, e));
    } else {
        console.warn(`Audio file for number ${number} not loaded or missing.`);
        // Fallback to TTS if audio file fails?
        // speakNumber(number); 
    }
}
// --- End Audio Playback Function ---
